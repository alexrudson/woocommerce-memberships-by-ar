<?php
/**
 * Plugin Name: WooCommerce Memberships v 1.25.11 modificado por AR
...
// Desativa atualizações automáticas do plugin
add_filter('auto_update_plugin', function($update, $plugin) {
    if ($plugin === 'woocommerce-memberships/woocommerce-memberships.php') {
        return false;
    }
    return $update;
}, 10, 2);

// Remove notificações de atualização
add_filter('site_transient_update_plugins', function($transient) {
    if (isset($transient->response['woocommerce-memberships/woocommerce-memberships.php'])) {
        unset($transient->response['woocommerce-memberships/woocommerce-memberships.php']);
    }
    return $transient;
});

// Adiciona mensagem informativa na página de plugins
add_action('admin_notices', function() {
    global $pagenow;
    if ($pagenow === 'plugins.php') {
        ?>
        <div class="notice notice-info">
            <p><strong>WooCommerce Memberships:</strong> As atualizações automáticas foram desativadas para manter a versão atual funcionando.</p>
        </div>
        <?php
    }
});

// Força a versão atual do plugin e previne atualizações
...
// Remove o plugin da lista de atualizações disponíveis
add_filter('plugins_api', function($result, $action, $args) {
    if ($action === 'plugin_information' && isset($args->slug) && $args->slug === 'woocommerce-memberships') {
        return new WP_Error('plugins_api_failed', 'Atualizações desativadas para este plugin.');
    }
    return $result;
}, 10, 3);
...
add_action('plugins_loaded', function() {
    load_plugin_textdomain('woocommerce-memberships', false, dirname(plugin_basename(__FILE__)) . '/i18n/languages/');
});